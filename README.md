...
**触发规则**:每次推送到 `master` 都会构建镜像(同时更新 `latest`);推送 `v*.*.*` 标签时额外生成语义版本标签。Actions 页面也可手动 Run workflow。
...
**默认拉取 latest**:不设 `BACKEND_IMAGE`/`WEB_IMAGE` 时拉取 `latest`,每次推送自动更新。如需锁定某版本,在服务器 `.env` 里用 `BACKEND_IMAGE` / `WEB_IMAGE` 指定带版本号的标签:
...
> 若 pull 到旧缓存,加 `--force-recreate` 重新 `up -d`。
